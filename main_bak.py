...
def create_data(output_index=0,
    ...
    if mode == "increment":
        ...
        output_size = 13 # fixed output size at 13 for one_extra mode
    ...
    if mode == "increment":
        ...
        data_output_np_new = data_output_np[:, :output_size]
        sel = output_array_selection[output_index]
        assert (sel > 12) # check
        data_output_np_new[:, 12] = data_output_np[:, (sel-1)] # replace column 13

        #print("data_output_np_new.shape", data_output_np_new.shape)
        #print("data_output_np_new[1,:]", data_output_np_new[1, :])

        data_output_np = data_output_np_new
...
def creat_dnn_model(output_index = 0,
    ...
    if mode == "one_extra": # fixed at 13
        model = LargerNN(input_size, hidden_sizes, 13).to(device)
    else:
...
# backup filtered_output.pt / filtered_input.pt
shutil.copyfile(os.path.join(path_data, "filtered_output.pt"),
                os.path.join(path_save, "filtered_output.pt"))

shutil.copyfile(os.path.join(path_data, "filtered_input.pt"),
                os.path.join(path_save, "filtered_input.pt"))




### mode = increment
for index in range(9): #[0, 9): 0,1,2 ..., 8


    print("========================== start incremental training index = ", index)
    ...
    print("========================== end incremental training index = ", index)
    print()
...
### mode = one_extra
for index in range(1, 9): # 1,2,3, ..., 8
...
best_loss = 1.0
best_index = 0
# i = 0 -> 8: increment mode, increment_12 -> increment_20
# i = 9 -> 16: one_extra mode, index_13 -> index_20
